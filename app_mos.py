...
def page_audio_evaluation():
    ...
    if current_audio_index < length_evalset:
        ...
        if f"confirmed_evaluation_{current_audio_index}" not in st.session_state:
            st.session_state[f"confirmed_evaluation_{current_audio_index}"] = False

        confirmed_evaluation = st.checkbox(
            "Are you satisfied with your evaluation of this audio?",
            key=f"confirmed_evaluation_{current_audio_index}"
        )
        ...
        # button for next
        if st.button("Next", disabled=not confirmed_evaluation):
            #if not st.session_state.confirmed_evaluation:
            ...
            del st.session_state[f"confirmed_evaluation_{current_audio_index}"]
            #st.session_state.confirmed_evaluation = False
            #del st.session_state["confirmed_evaluation"]
